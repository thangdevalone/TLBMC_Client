...
export const Course = () => {
    ...
    const fetchData = async () => {
        const res = await courseApi.getCourse()  as unknown as InfoCourse[];
        if(data){
            res.forEach((item)=>{
                if(item.id===data.id){
                    setData(item)
    ...
    return (
        <div className=" h-full mb-5 grid md:gap-5 sm:gap-2 xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2">
